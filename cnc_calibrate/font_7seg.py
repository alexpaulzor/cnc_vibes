"""Seven-segment block-font digit renderer (vendored from the cnc_vibes laser
calibration lesson).

Renders digits 0-9 as a list of line segments suitable for tracing with a laser
or CNC engraving operation. No font-library dependency.

Each digit occupies a W x H box where W = H / 2. Segments are labeled a-g per the
standard 7-segment-display convention:
...
def render_text(
    ...
    """Render a string of digits as line segments starting at (origin_x, origin_y),
    growing in +X. Non-digit characters are skipped (leaving their pitch as a gap)."""
    w = height / 2
    pitch = w + spacing
    out = []
    for i, ch in enumerate(text):
        if ch in _DIGIT_SEGMENTS:
