...
# CRITICAL: phase5 captures phase2's CELL_W/COLS/etc via `from import` at its
# import time. Our small-puzzle overrides only work if phase5 has NOT been
# loaded before we mutate phase2. The assert guarantees correct import order.
assert "diagram_word_phase5" not in sys.modules, (
    "phase6_small must be imported before diagram_word_phase5 — its overrides "
    "of phase2 constants only propagate if phase5 hasn't already been loaded."
)
...
# Phase5's module-level computations (LETTER_CLEARANCE_PX = TAB_CIRCLE_R, etc.)
# also evaluate against the overridden values; no post-hoc fix-ups needed.
...
def emit_gcode(pieces: list[dict], material: dict, word: str) -> str:
    ...
    lines = [
        ...
        f";",
        f"; ASSUMES: Z is already at focal height in your WCS. This file emits",
        f"; no Z motion. Set focus by hand or via interactive_cal before cutting.",
        f";",
        f";HEAD: laser",
