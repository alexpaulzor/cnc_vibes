...
The current script targets **cutting** (G3 circle, M4 dynamic power). Grayscale raster engraving needs a different test pattern: parallel raster lines at varying power, fixed feed, single pass, with the laser modulating power based on image intensity.

The parameters for grayscale are:
- **Power**: ~10-30% range (varies image-to-image; too high = burn-through, too low = no contrast).
- **Feed**: 3000-6000 mm/min typical (much faster than cutting).
- **Line spacing**: 0.1-0.2 mm (matched to laser dot size).
- **Passes**: 1.

**Status:** Not yet supported by this script. The cleanest path forward is either:
- (a) Add `--mode engrave` to this script that emits a raster patch instead of a circle cut (a small TODO — let me know to prioritize it), or
- (b) Use lesson 3b's static calibration with raster-line patterns.

Until then, calibrate grayscale empirically with small test patches generated by your image-engrave tool, or wait for the engrave mode.
