...
def emit_label_gcode(
    ...
    cx = slot_x + DEFAULT_SLOT_W / 2
    label_origin_x = cx - label_w / 2
    label_origin_y = slot_y + DEFAULT_SLOT_H - digit_height - 2  # near top of slot
...
def emit_circle_cut_gcode(
    ...
    r = circle_dia / 2
    cx = slot_x + DEFAULT_SLOT_W / 2
    cy = slot_y + DEFAULT_SLOT_H / 2 - 4  # below the label
...
def emit_iteration_gcode(
    ...
    lines.extend(emit_circle_cut_gcode(slot_x, slot_y, circle_dia, params))
    return lines, (cx, cy)
...
def _prompt_evaluate_and_adjust(
    ...
    Returns (next_params or None to stop, outcome, notes).
    """
    print("\n--- Evaluate the cut ---")
...
def main() -> int:
    ...
    if not args.dry_run:
        ...
        try:
            ...
            # Set laser mode + units + absolute
            for setup in ("$32=1", "G21", "G90", "M5"):
                _send_line(ser, setup)
    ...
    try:
        for iter_n in range(1, args.max_iterations + 1):
            ...
            if args.dry_run:
                ...
                next_params, outcome, notes = _prompt_evaluate_and_adjust(params)
            logs.append(
                IterationLog(
                    n=iter_n,
